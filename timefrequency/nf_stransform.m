function tfRes = nf_stransform(data, Fs, factor, verbose)
...
% 4) verbose: print verbose output? 0 or 1, defaults to 0
%
...
if nargin < 3 || isempty(factor)
    factor = 1;
    if verbose == 1
        disp('[nf_stransform]: setting factor to 1 (default)');
    end
end
if nargin < 2 || isempty(Fs) || isempty(data)
    error('nf_stransform:InputsRequired',...
...
disp('[nf_stransform]: beginning stockwell transform');
...
disp('[nf_stransform]: stockwell transform complete');
