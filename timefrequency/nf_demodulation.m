function tfRes = nf_demodulation(data,Fs,freqs,lowpassF,order,verbose)
...
if nargin<5 || isempty(order)
    order = 3;
    if verbose == 1
        disp('[nf_demodulation]: setting filter order = 3 (default)');
    end
end
if nargin<4 || isempty(lowpassF)
    lowpassF = 2;
    if verbose == 1
        disp('[nf_demodulation]: setting low-pass filter at 2 Hz (default)');
    end
end
if nargin<3 || isempty(freqs)
    freqs = 1:1:floor(Fs/2);
    if verbose == 1
        disp('[nf_demodulation]: setting freqs to 1:1:Fs/2 (default)');
    end
end
if nargin<2 || isempty(Fs) || isempty(data)
    error('[nf_demodulation]: at least a signal and sampling rate are required');
end
...
%demodulate data
disp('[nf_demodulation]: beginning complex demodulation');
...
disp('[nf_demodulation]: complex demodulation complete');
