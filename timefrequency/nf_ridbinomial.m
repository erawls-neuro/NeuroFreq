function tfRes = nf_ridbinomial(data,Fs,fRes,maxlags,makePos,verbose)
...
if nargin<5 || isempty(makePos)
    makePos=0;
    if verbose == 1
        disp('[nf_ridbinomial]: returning both positive and negative values (default)');
    end
end
if nargin<4 || isempty(maxlags)
    maxlags=2*nTimes;
    if verbose == 1
        disp('[nf_ridbinomial]: setting window length to 2*N (default)');
    end
else
...
if nargin<3 || isempty(fRes)
    ...
    if verbose == 1
        disp('[nf_ridbinomial]: setting frequencies output to 2*N (default)');
    end
else
    fout = 0:fRes:Fs/2; %user-specified resolution
end
if nargin<2 || isempty(Fs) || isempty(data)
    error('nf_ridbinomial:InputsRequired',...
...
%progress
disp('[nf_ridbinomial]: beginning binomial rid');
...
disp('[nf_ridbinomial]: binomial rid complete');
