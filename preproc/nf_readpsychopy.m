...
for variableIndex = 1:width(raw)
    ...
    if groupIndex == 0
        group = struct();
        group.mask = mask;
        group.names = {names{variableIndex}};
...
for flaggedIndex = 1:numel(flaggedLocalRows)
    variableIndices = find(evidence(flaggedLocalRows(flaggedIndex), :));
...
for localIndex = 1:numel(sourceRows)
    ...
    for textIndex = 1:numel(rowTexts)
        normalizedText = lower(strtrim(rowTexts{textIndex}));
        if any(strcmp(normalizedText, sentinels))
...
if isstring(value)
    ...
    for itemIndex = 1:numel(rowValue)
        if ~ismissing(rowValue(itemIndex))
            texts{end + 1} = char(rowValue(itemIndex));
        end
    end
    return
end
...
if iscategorical(value)
    ...
    for itemIndex = 1:numel(rowValue)
        if ~ismissing(rowValue(itemIndex))
            texts{end + 1} = char(string(rowValue(itemIndex)));
        end
    end
    return
end
...
for variableIndex = 1:width(raw)
    ...
    metadataNames{end + 1} = names{variableIndex};
    sourceRows(end + 1) = presentRows(1);
end
end
...
fprintf(['[NeuroFreq] nf_readpsychopy retained %d of %d raw rows using ' ...
...
if ~isempty(info.flagged.retainedSourceRows)
    fprintf(['[NeuroFreq] %d possible setup/seed rows were flagged and ' ...
...
if ~isempty(info.metadataVariables) && options.PropagateMetadata
    fprintf('[NeuroFreq] Propagated run metadata: %s.\n', ...
