function PAC = nf_mvlpac_cross(phas, pow, nperm, phaseCh, ampCh, use_mex)
% NF_MVLPAC_CROSS    Cross-channel MVL-based PAC (phase from phaseCh, amplitude from ampCh).
%
%   phas    : [nChan_phase_total x nLoF x nTimes x nTrial]
%   pow     : [nChan_amp_total   x nHiF x nTimes x nTrial]
%   nperm   : number of time-shift surrogates (0 = raw MVL)
%   phaseCh : vector of indices into phas channels (default: all)
%   ampCh   : vector of indices into pow channels  (default: all)
%   use_mex : logical, use MVL MEX and timeshift MEX if available
%
%   Output:
%   PAC : [nPhaseChan x nAmpChan x nLoF x nHiF x nTrial]
%         If nperm == 0  -> raw MVL
%         If nperm  > 0  -> z-scored MVL
...
if ndims(phas) ~= 4
    error('nf_mvlpac_cross: phas must be [nChan x nLoF x nTimes x nTrial].');
end

if ndims(pow) ~= 4
    error('nf_mvlpac_cross: pow must be [nChan x nHiF x nTimes x nTrial].');
end
...
if nTimes1 ~= nTimes2
    error('nf_mvlpac_cross: phas and pow must share the same nTimes.');
end

if nTrial1 ~= nTrial2
    error('nf_mvlpac_cross: phas and pow must share the same nTrial.');
end
    ...
    for iAmp = 1:nAmpChan
        ...
        for f1 = 1:nLoF
            ...
            for f2 = 1:nHiF
                ...
                mvl_obs_3d = mvl_kernel(phase_samples, power_samples, use_mex);
                mvl_obs = reshape(mvl_obs_3d, [1, nTrial]);
                ...
                if nperm > 0
                    ...
                    for ip = 1:nperm
                        ...
                        mvl_perm_3d = mvl_kernel(phase_samples, power_perm, use_mex);
                        mvl_perm = reshape(mvl_perm_3d, [1, nTrial]);
                        
                        sum_null  = sum_null  + mvl_perm;
                        sum2_null = sum2_null + mvl_perm .^ 2;
                    end
