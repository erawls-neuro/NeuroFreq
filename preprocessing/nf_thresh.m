...
for detectorIndex = 1:numel(options.detectors)
    ...
    provenance(end + 1) = contract; %#ok<AGROW>
    ...
    switch method
        case 'jointprobability'
            [EEG, methodChannelMask, methodEpochMask, methodDetail] = ...
                nf_detect_joint_probability(EEG, settings);
            functionNames{end + 1} = 'pop_jointprob'; %#ok<AGROW>
        case 'kurtosis'
            ...
            functionNames{end + 1} = 'pop_rejkurt'; %#ok<AGROW>
        otherwise
...
for trialIndex = 1:trials
    ...
    if directEpochMask(trialIndex)
        applicable{end + 1} = repair.detectedEpochAction; %#ok<AGROW>
    end
    if excessMask(trialIndex)
        applicable{end + 1} = repair.excessChannelAction; %#ok<AGROW>
    end
    if frontalMask(trialIndex)
        applicable{end + 1} = repair.frontalAction; %#ok<AGROW>
    end
    if any(channelMask(:, trialIndex))
        applicable{end + 1} = repair.sparseChannelAction; %#ok<AGROW>
    end
...
for index = 1:numel(functionNames)
    ...
    if ~ismember(fileType, [2 3 6])
        missing{end + 1} = functionNames{index}; %#ok<AGROW>
    end
end
if ~isempty(missing)
