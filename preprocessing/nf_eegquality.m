...
for epochIndex = 1:EEG.trials
    ...
    for startIndex = 1:numel(starts)
        ...
        epochValues(globalIndex) = epochIndex; %#ok<AGROW>
        startValues(globalIndex) = starts(startIndex); %#ok<AGROW>
        stopValues(globalIndex) = starts(startIndex) + targetSamples - 1; %#ok<AGROW>
        globalValues(globalIndex) = globalIndex; %#ok<AGROW>
    end
end
...
if ~ismatrix(weights) || ~ismatrix(sphere) || ...
...
for index = 1:numel(fields)
    ...
    if isfield(classification, fieldName) && ...
        ...
        fieldAvailable = true;
        candidate = [candidate reshape(classification.(fieldName), 1, [])]; %#ok<AGROW>
    end
end
if fieldAvailable
...
elseif ~contains(lower(channels.method), 'prep')
...
for index = 1:numel(indices)
    ...
    if channelIndex <= numel(rawLabels)
        labels{end + 1} = rawLabels{channelIndex}; %#ok<AGROW>
        rawMappedIndices(end + 1) = channelIndex; %#ok<AGROW>
        rawMappedCount = rawMappedCount + 1;
    elseif isempty(rawLabels) && channelIndex <= numel(finalLabels)
        labels{end + 1} = finalLabels{channelIndex}; %#ok<AGROW>
        finalMappedCount = finalMappedCount + 1;
    else
        unresolvedIndices(end + 1) = channelIndex; %#ok<AGROW>
    end
end
...
for inputIndex = 1:nargin
    labels = [labels nf_to_cellstr(varargin{inputIndex})]; %#ok<AGROW>
end
...
for index = 1:numel(activeDetectors)
    ...
    normalizedDetectors{end + 1} = detector; %#ok<AGROW>
end
...
if ~(isnumeric(candidate) || islogical(candidate)) || ...
        ~isreal(candidate) || ~ismatrix(candidate)
    return
end
...
if channelActions.available && channelActions.nDetected > 0
    ...
    if badFraction > thresholdValue %#ok
...
if nf_topography_available(locations) && coordinatesAvailable
    try
        ...
        for categoryIndex = 1:numel(categoryNames)
            ...
            legendHandles(end + 1, 1) = handle; %#ok<AGROW>
            legendNames{end + 1} = displayNames{categoryIndex}; %#ok<AGROW>
        end
...
for firstIndex = 1:detectorsPerLine:numel(parts)
    ...
    lines{end + 1} = [prefix strjoin(parts(firstIndex:lastIndex), ' | ')]; %#ok<AGROW>
end
end
