...
addParameter(parser, 'verbose', 'off');
...
fprintf( ...
    ['[nf_cleanic]: ICA rank check: %d/%d channels using covariance ' ...
    'eigenvalues > %.1e.\n'], ...
    rankBeforeRepair, ...
    training.nbchan, ...
    rankTolerance);
...
if rankBeforeRepair < training.nbchan
    ...
    fprintf( ...
        ['[nf_cleanic]: Rank deficiency detected; removing %d dependent ' ...
    ...
    fprintf( ...
        '[nf_cleanic]: ICA data are full rank; channel rank repair skipped.\n');
end
if strcmp(method, 'faster')
...
EEG.icaact = [];
nf_validate_decomposition(EEG);

componentCount = size(EEG.icaweights, 1);
training.data = [];
training.icaact = [];
if isfield(training, 'specdata')
    training.specdata = [];
end
if isfield(training, 'specicaact')
    training.specicaact = [];
end
if strcmp(method, 'iclabel')
...
[EEG, postCleanDecomposition] = nf_detach_ica_for_epoching(EEG);
EEG = eeg_checkset(EEG);
EEG = nf_restore_ica_after_epoching(EEG, postCleanDecomposition);
if behaviorInfo.present
    EEG = nf_set_eeg_behavior(EEG, inputBehavior);
end
...
if tmprank ~= rankValue
    rankValue = min(rankValue,tmprank);
end
end
...
training = nf_finalize_decomposition(training);
...
[reportFile, temporaryReport, reportCleanup] = ...
    nf_prepare_adjust_report(options.reportFile); %#ok
[~, environmentCleanup] = ...
    nf_prepare_vendor_environment(); %#ok
...
[classifierEEG, classifierDecomposition] = ...
    nf_detach_ica_for_epoching(classifierEEG);
...
classifierEEG = nf_restore_ica_after_epoching( ...
    classifierEEG, classifierDecomposition);
...
function EEG = nf_finalize_decomposition(EEG)
if ~isfield(EEG, 'icaweights') || isempty(EEG.icaweights) || ...
        ~isfield(EEG, 'icasphere') || isempty(EEG.icasphere)
    error('nf_cleanic:ICAFailed', ...
        'ICA did not return weights and a sphere matrix.');
end
...
requiredFields = {'icaweights', 'icasphere', 'icawinv', 'icachansind'};

for fieldIndex = 1:numel(requiredFields)
    fieldName = requiredFields{fieldIndex};

    if ~isfield(EEG, fieldName) || isempty(EEG.(fieldName))
        error('nf_cleanic:ICAFailed', ...
...
if ~isfield(EEG, 'nbchan') || isempty(EEG.nbchan) || ...
        ~isscalar(EEG.nbchan) || ~isfinite(EEG.nbchan)
    error('nf_cleanic:ICAFailed', ...
        'The EEG channel count is unavailable or invalid.');
end
...
channelIndices = double(EEG.icachansind(:));
componentCount = size(EEG.icaweights, 1);
sphereRank = size(EEG.icasphere, 1);
sphereChannels = size(EEG.icasphere, 2);
validChannelIndices = all(isfinite(channelIndices)) && ...
    all(channelIndices == round(channelIndices)) && ...
    all(channelIndices >= 1) && ...
    all(channelIndices <= EEG.nbchan) && ...
    numel(unique(channelIndices)) == numel(channelIndices);
validMultiplication = size(EEG.icaweights, 2) == sphereRank;
validInverse = size(EEG.icawinv, 1) == sphereChannels && ...
    size(EEG.icawinv, 2) == componentCount;
fullSquareDecomposition = componentCount == EEG.nbchan && ...
    sphereRank == EEG.nbchan && ...
    sphereChannels == EEG.nbchan && ...
    numel(channelIndices) == EEG.nbchan;

if validChannelIndices == false || ...
        validMultiplication == false || ...
        validInverse == false || ...
        fullSquareDecomposition == false
    error('nf_cleanic:NonSquareICA', ...
        ['The final ICA matrices or channel indices do not describe one ' ...
        'square decomposition across every retained EEG channel.']);
end
end
