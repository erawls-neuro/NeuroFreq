...
if strcmpi(mode, 'stat') == 1
    error('nf_corr_TFCE:BadInput', 'Input TF struct appears to be stat output. Correlation requires raw subject/trial-level data.');
end
...
if isfield(tfset, 'power') == 1
    if isstruct(tfset.power) == 1
        ...
        if isempty(fns) == 0
            groupList{end + 1, 1} = 'power'; %#ok<AGROW>
            fieldMap.power = fns;
        end
    end
end
...
if isfield(tfset, 'phase') == 1
    if isstruct(tfset.phase) == 1
        ...
        if isempty(fns) == 0
            groupList{end + 1, 1} = 'phase'; %#ok<AGROW>
            fieldMap.phase = fns;
        end
    end
end
...
function [Xobs, nObs, nChan, nFreq, nTime] = local_standardize_singletrial_obsfirst(X, tfset, chanlocs, faxis, taxis)
