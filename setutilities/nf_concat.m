...
disp('[nf_concat]: concatenation complete');
...
elseif isstruct(files)
    if is_dir_listing(files)
        for fileIndex = 1:numel(files)
            inputItems{end + 1} = fullfile(files(fileIndex).folder, files(fileIndex).name); %#ok<AGROW>
        end
    else
        for fileIndex = 1:numel(files)
            inputItems{end + 1} = files(fileIndex); %#ok<AGROW>
        end
    end
else
...
if ischar(item)
    if has_wildcard(item)
        ...
        for listingIndex = 1:numel(listing)
            inputItems{end + 1} = fullfile(listing(listingIndex).folder, listing(listingIndex).name); %#ok<AGROW>
        end
    else
...
elseif isstruct(item)
    if is_dir_listing(item)
        for listingIndex = 1:numel(item)
            inputItems{end + 1} = fullfile(item(listingIndex).folder, item(listingIndex).name); %#ok<AGROW>
        end
    else
        for structIndex = 1:numel(item)
            inputItems{end + 1} = item(structIndex); %#ok<AGROW>
        end
    end
else
...
if isempty(starHit) %#ok
    hasStar = false;
else
    hasStar = true;
end

if isempty(questionHit) %#ok
    hasQuestion = false;
else
    hasQuestion = true;
end
...
for variableIndex = 1:numel(variableNames)
    ...
    if is_neurofreq_like(candidate)
        candidateNames{end + 1} = variableNames{variableIndex}; %#ok<AGROW>
    end
end
...
for knownIndex = 1:numel(knownTrialFields)
    ...
    if isfield(TF, fieldName)
        if is_candidate_data_field(TF, fieldName, opts, true)
            dataFields{end + 1} = fieldName; %#ok<AGROW>
        end
    end
end
...
for inputIndex = 1:nInput
    ...
    for groupIndex = 1:numel(groupFirstIndices)
        ...
        if vectors_equal_with_tolerance(timeVectors{inputIndex}, timeVectors{representativeIndex}, tolerance)
            ...
            groupCounts(groupIndex) = groupCounts(groupIndex) + 1; %#ok<AGROW>
            assigned = true;
            break;
        end
    end
    ...
    if ~assigned
        groupFirstIndices(end + 1) = inputIndex; %#ok<AGROW>
        groupCounts(end + 1) = 1; %#ok<AGROW>
        groupLengths(end + 1) = numel(timeVectors{inputIndex}); %#ok<AGROW>
...
for partIndex = 1:numel(parts)
    ...
    for fieldIndex = 1:numel(thisFields)
        if ~any(strcmp(allFields, thisFields{fieldIndex}))
            allFields{end + 1} = thisFields{fieldIndex}; %#ok<AGROW>
        end
    end
end
...
report.created = datestr(now, 30); %#ok
